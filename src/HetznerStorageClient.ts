...
export class HetznerStorageClient {
   ...
   */
  static create(config: Partial<StorageConfig> & { type: StorageConfig['type'] }): BaseStorageProvider {
    if (!config.type) {
    ...
    switch (config.type) {
      case 'box': {
        ...
        if (!mergedConfig.username || !mergedConfig.password) {
          throw new Error(
            'Username and password are required for Storage Box. ' +
      ...
      case 'share': {
        ...
        if (!mergedConfig.username || !mergedConfig.password) {
          throw new Error(
            'Username and password are required for Storage Share. ' +
        ...
        if (!mergedConfig.instance) {
          throw new Error(
            'Instance is required for Storage Share. ' +
      ...
      case 'object': {
        ...
        if (!mergedConfig.accessKeyId || !mergedConfig.secretAccessKey) {
          throw new Error(
            'accessKeyId and secretAccessKey are required for Object Storage. ' +
  ...
  static createBoxClient(
    ...
    const envConfig = loadStorageBoxEnv();
    const mergedConfig = mergeWithEnv({ ...config, type: 'box' as const }, envConfig) as StorageBoxConfig;
  ...
  static createShareClient(
    ...
    const envConfig = loadStorageShareEnv();
    const mergedConfig = mergeWithEnv({ ...config, type: 'share' as const }, envConfig) as StorageShareConfig;
  ...
  static createObjectClient(
    ...
    const envConfig = loadObjectStorageEnv();
    const mergedConfig = mergeWithEnv({ ...config, type: 'object' as const }, envConfig) as ObjectStorageConfig;
