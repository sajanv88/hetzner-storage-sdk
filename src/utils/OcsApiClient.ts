...
import type { CreateShareOptions, OCSShareData, OCSShareResponse, StorageError } from '../types';
...
export class OcsApiClient {
    ...
    async getShare(shareId: string): Promise<OCSShareData> {
        const response = await this.axiosInstance.get<OCSShareResponse>(`/shares/${shareId}`);
        const data = response.data.ocs.data;
