...
import { AuthType, type FileStat, type WebDAVClient, createClient } from 'webdav';
import type {
    DownloadOptions,
    FileMetadata,
    SnapshotInfo,
...
export class StorageBoxProvider extends BaseStorageProvider {
    ...
    constructor(config: StorageBoxConfig) {
        ...
        if (config.robotUsername && config.robotPassword) {
            this.robotClient = new RobotApiClient(config.robotUsername, config.robotPassword);
        }
     ...
     */
    async toggleServices(services: StorageBoxServiceToggle): Promise<StorageBoxDetails> {
        if (!this.robotClient) {
            throw new Error(
                'Robot API credentials not provided. Set robotUsername and robotPassword in config.'
            );
        ...
        return await this.robotClient.toggleServices(this._config.storageBoxId, services);
    }
    ...
    async createSnapshot(comment?: string): Promise<SnapshotInfo> {
        ...
        return await this.robotClient.createSnapshot(this._config.storageBoxId, comment);
    }
    ...
    async deleteSnapshot(snapshotName: string): Promise<void> {
        ...
        return await this.robotClient.deleteSnapshot(this._config.storageBoxId, snapshotName);
    }
    ...
    async revertToSnapshot(snapshotName: string): Promise<void> {
        ...
        return await this.robotClient.revertToSnapshot(this._config.storageBoxId, snapshotName);
    }
}
