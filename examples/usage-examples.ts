...
import {
    ...
    type StorageBoxProvider,
    type StorageShareProvider,
} from '../dist';
...
async function example6_PublicLinkSharing() {
    ...
    try {
        ...
        await client.uploadFile(Buffer.from('Shared document content'), '/shared-document.pdf');
...
async function example7_UserGroupSharing() {
    ...
    try {
        ...
        await client.uploadFile(Buffer.from('Team document'), '/Team/project.docx');
...
async function example8_BackupSystem() {
    ...
    try {
        ...
        for (const file of filesToBackup) {
            await client.uploadFile(Buffer.from(file.content), `${currentBackupDir}/${file.name}`);
